		...
		<?php
			...
			*/
			...
			echo "<u><b>Unpaid Shares</b></u> ".
			     "<span id='tt'><img src='images/questionmark.png' height='15px' width='15px' title='Submitted shares between the last $nConfirms confirms ".
			     "block until now.'></span>".
			     "<br>";

			echo "Your Valid: <b><i>".($totalUserShares / 1)."</i> <font size='1px'></font></b><br/>";
			echo "Pool Valid: <b><i>".($totalOverallShares/1)."</i> <font size='1px'></font></b><br/><br>";
			...
			if ($nextblock_shares < 0) { $nextblock_shares = "$totalOverallShares, $pending_shares"; }
			...
			//echo "<u><b>Round Estimate</b></u><font size='1'></font></u><br><b><i>".round($estimatedTotaEarnings, 8)."</i> <font size='1px'>LTC</font></b><br><br>";
...
?>
	...
	<p>
		...
		<?php
		    if($userInfo->tz==NULL)
                $sdate=convertTimeZone($settings->getsetting('statstime'),$systimezone,'UTC');		    
            else
			    $sdate=convertTimeZone($settings->getsetting('statstime'),$systimezone,$userInfo->tz);
			echo "".$sdate->format("h:i:s A e")."<br/>"; 
			?>
		<br>
