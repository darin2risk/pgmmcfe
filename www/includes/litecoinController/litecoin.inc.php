 ...
 *
 * If you find this library useful, your donation of Litecoins to address
 ...
 * @author theymos - theymos @ http://Litecoin.org/smf
 * @author Refactored for LITECOIN by Lavajumper
 */

define("Litecoin_ADDRESS_VERSION", "00");// this is a hex byte
/**
...
class Litecoin {
  ...
  /*
   * Litecoin utility functions by theymos
   * Via http://www.Litecoin.org/smf/index.php?topic=1844.0
...
class LitecoinClient extends jsonrpc_client {
  ...
  /**
   * Returns boolean true if server is trying to generate Litecoins, false otherwise.
   *
  ...
  /**
   * Returns transaction list from litecoind reference implementation
   *
   * REQUIRED by pushpoold
   * @param string $account - account number
   * @param int count - number of accounts to return
   * @param int from - starting account offset
   * @returns array of transaction
   *
   * @author lavajumper
   */
  public function listtransactions($account, $count=10, $from=0){
	//echo("listtransactions: $account, $count,$from\n");
	
	return $this->query("listtransactions", $account, $count,$from);
  }
  
  public function getworkex($data=NULL,$coinchain=NULL){
		throw new LitecoinClientException("getworkex not implemented yet");
	return(NULL);
  }
  
  public function getnetworkhashps(){
	return($this->query("getnetworkhashps"));
  }
