...
if (isset($_POST["username"])) { $user = db_real_escape_string($_POST["username"]); } else { $user = NULL; }
if (isset($_POST["password"])) { $pass = db_real_escape_string($_POST["password"]); } else { $pass = NULL; }

//Check the supplied username & password with the saved username & password
//$sql="SELECT id,secret,pass,accountlocked,accountfailedattempts FROM webusers WHERE username='".$user."'";
//echo $sql;
$checkPassQ = db_query("SELECT id, secret, pass, accountlocked, accountfailedattempts FROM webusers WHERE username = '".$user."' LIMIT 1");
$checkPass = db_fetch_object($checkPassQ);
//var_dump($checkPass);
	...
	if($checkPass->accountlocked < time()){
		...
		if($checkPass->accountfailedattempts < 5){
			...
			if($dbHash == $inputHash){
				...
				db_query("UPDATE webusers SET sessiontimeoutstamp = '".$timeoutStamp."', loggedip = '".$ip."' WHERE id = ".$userExists);
